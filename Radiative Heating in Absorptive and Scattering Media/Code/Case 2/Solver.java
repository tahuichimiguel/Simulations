...
class Solver {
	...
	public void InitializeIPlusIMinus(int maxIter){
		...
		IPlus[0]=(-2*beta*sigma*Math.pow(Tcool,4)/Math.PI + 2*kappa*sigma*Math.pow(Tcool,4)/Math.PI 
				  + scattC*(sigma*Math.pow(Tcool,4)/Math.PI))*dels+sigma*Math.pow(Tcool,4)/Math.PI;
		
		//Last Indexed I- Element (To left of the hot wall)
		IMinus[nodecount-1]=(2*beta*sigma*Math.pow(Thot,4)/Math.PI - 2*kappa*sigma*Math.pow(Thot,4)/Math.PI 
				  - scattC*(sigma*Math.pow(Thot,4)/Math.PI))*(-dels)+(sigma*Math.pow(Thot,4)/Math.PI);
		...
		while(count<maxIter){
			...
			for (int i = 1;i<nodecount; i++){
				...
				temp = (-2*beta*IPlus[i-1] + 2*kappa*sigma*Math.pow(T,4)/Math.PI + scattC*(IMinus[i-1]+IPlus[i-1]))*dels + IPlus[i-1];
			...
			for (int i = nodecount-2;i>-1; i--){
				...
				temp = (2*beta*IMinus[i+1] - 2*kappa*sigma*Math.pow(T,4)/Math.PI - scattC*(IMinus[i+1]+IPlus[i+1]))*(-dels)+IMinus[i+1];
